...
Trotzdem kommt diesem Verfahren keine kleinere Bedeutung zu, denn es kann, wie die Röntgendiagnostik auch, Menschenleben retten und dies auf eine effiziente und materialschonende Weise.\\ 
...
Mithilfe dessen kann auch Tiefe des Prüfkörpers ermittelt werden: da der Schall stets eine materialspezifische Geschwindigkeit $c_\mathrm{Material}$  \footcite{schallgeschwindigkeiten}  %TODO Quelle
...
Allerdings kann auch durch die verwendete Methode selbst eine Einschränkung erfolgen. So ist z.B. beim Prüfen mit Ultraschall die Materialdicke - abhängig vom verwendeten Prüfgerät - auf fünf bis acht Meter beschränkt, da sonst Ungenauigkeiten entstehen könnten. \footcite{ultraschall} \\ %TODO bessere Form Gedankenstrich oder Satz umstellen
...
Für unsere Simulation sind wir dabei von einer exponentiellen Funktion $I = \frac{I_0}{1.05^{\tfrac{s}{c}}}$ zur Berechnung der Intensität $I$, mit $I_0$ als Ausgangsintensität, $s$ als den zurückgelegten Weg und $c$ für die materialspezifische Schallgeschwindigkeit, ausgegangen.  %TODO den Weg oder dem Weg
...
%TODO bessere Überschrift: Grundlegend verwendete Prozedur zur Reflektion eines Strahls
...
Um dies zu ermöglichen, muss das Programm beliebige, komplexe und zweidimensionale Geometrien umsetzen können. %TODO Kommas und und ganz rauslassen -> alles Attribute
...
\subsubsection{Scansimulation mit Einfach- und Mehrfachstrahl} %TODO besserer Titel?
...
Um diesen Fall ebenfalls abzudecken, ist eine zweite Version dieser Prozedur verfasst. %TODO Synonym verfasst -> Bedeutung nicht gleich offensichtlich?
...
\newpage
\subsubsection{Auswertung der Messergebnisse} %TODO verweis Struktogramm Anhang -> wenn eingefügt
...
Durch die Annahme, dass jeder Strahl einen gewissen Teil der kinetischen Energie, die in Form einer Schallwelle in den Körper abgegeben wird, repräsentiert, sollten fast gleichzeitige Treffer nicht als solche wahrgenommen werden, sondern als ein Treffer mit höherer Intensität. %%todo ohne erklärung Welle?
Die Notwendigkeit einer weiteren Zusammenfassung der Messwerte ist dadurch gegeben. 
Dies erfolgt in der Methode \texttt{processScan}. \par %%todo einheitliche Schriftart für funktionen, Satz notwendig?
...
Nun wird aus dieser Liste an Differenzen die Stelle gesucht, an der der Quotient aus zwei aufeinanderfolgenden Zeitdifferenzen am größten ist und der größere Wert der Zeitdifferenzen als sogenannter Zeitbereich definiert. %TODO Wortwiederholung wird
...
Ausgehend von einem Startwert werden alle folgenden Werte überprüft und so lange der selben Gruppe zugeordnet, bis sie nicht mehr innerhalb des Zeitbereichs liegen. %TODO derselben zusammen oder getrennt
Die Gruppe wird dann ausgewertet, indem eine durchschnittliche Intensität der Treffer sowie eine Durchschnittszeit errechnet wird. %TODO durchschnittszeit wird falsch getrennt
...
%TODO schreibweise überarbeiten -> Ende wenig zusammenhängen
...
Diesen Prozess möchten wir unterstützen.\\ %TODO umformulieren und verbessern
